        ...
        >
          ...
          <div className="flex flex-col lg:w-[45%] overflow-y-auto">
            <div className="p-6 lg:p-8 space-y-6">
              ...
              {/* ── Section 4: 推荐照片 ── */}
              {(similarData?.photos?.length > 0 || nearbyData?.photos?.length > 0) && (
                <section>
                  <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">
                    推荐照片
                  </h3>

                  {similarData?.photos?.length > 0 && (
                    <div className="mb-4">
                      <p className="text-xs text-gray-400 mb-2">相似照片</p>
                      <div className="flex gap-2">
                        {similarData.photos.map((p) => (
                          <Link
                            key={p.id}
                            to={`/photo/${p.id}`}
                  ...
                  {nearbyData?.photos?.length > 0 && (
                    <div className="mb-4">
                      <p className="text-xs text-gray-400 mb-2">附近照片</p>
                      <div className="flex gap-2">
                        {nearbyData.photos.map((p) => (
                          <Link
                            key={p.id}
                            to={`/photo/${p.id}`}
                            className="block w-1/3 aspect-square rounded-lg overflow-hidden bg-gray-100"
